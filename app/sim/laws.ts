...
export const LAW_DEFS: LawDef[] = [
  ...([
    ...
    ['lifeReserveTicks', 'Survival reserve', 72, 2, 150, 1, 'Emax = maintenance·N·reserve', 'Maximum time a full reserve can fund maintenance without food. One tick is 15 minutes.'],
    ['lifeTransitCapacity', 'Minimum travelling capacity', 0.5, 0.1, 1, 0.05, 'K = capacity · max(floor, H)', 'Prevents a brief crossing of clear air from immediately crushing a small colony. Feeding still requires habitat and charge.'],
    ['lifeDispersalIsolation', 'Dispersal isolation radius', 0.25, 0.1, 0.6, 0.01, 'no other colony within radius', 'Chord distance on the unit sphere. Only isolated colonies collect excess population for launch.'],
    ['lifeGatherTicks', 'Dispersal collection time', 4, 1, 20, 1, 'crowded ticks ≥ collection time', 'Excess creatures remain alive, consuming energy, until launch. Collection stops when crowding or isolation ends.'],
    ['lifeLaunchTicks', 'Dispersal impulse duration', 8, 1, 20, 1, 'launch lasts N ticks', 'Short crowding grace during launch. Afterwards ordinary wind drift and mortality resume.'],
    ['lifeLaunchSpeed', 'Dispersal impulse speed', 0.025, 0.005, 0.05, 0.005, 'tangent displacement per tick', 'A random launch direction added to wind, not a permanent ability to travel against it.'],
    ['lifeLaunchCost', 'Dispersal energy loss', 0.05, 0, 0.5, 0.01, 'child energy = transferred energy · (1−loss)', 'Launch is paid from the transferred creatures’ energy. It creates no population or energy and does not mutate traits.'],
